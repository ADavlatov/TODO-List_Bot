...
using TaskListBot.Commands;
using TaskListBot.Commands.AddNewTask;
using TaskListBot.Commands.TaskActions;
...
#pragma warning restore CA1031
    ...
    private async Task BotOnMessageReceived(Message message, ApplicationContext db)
    {
        ...
        async Task<Message> OnMessageReceived(ITelegramBotClient? bot, Message message)
        {
            ...
            string cache;
            if (Cache!.TryGetValue("TaskAction" + message.From!.Id, out cache!))
            {
            ...
            if (!Cache!.TryGetValue("Action" + message.From.Id, out cache!))
            {
    ...
    public static void SendTaskNotification(TaskObject? task)
    {
        if (task == null)
        {
           return; 
    ...
    private Task BotOnCallbackQueryReceived(CallbackQuery? callbackQuery, Message message, ApplicationContext db)
    {
